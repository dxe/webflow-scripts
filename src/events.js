...
function getAllChapters() {
  fetch(`https://adb.dxe.io/regions`)
    ...
    .then((regions) => {
        ...
        chapters.forEach((chapter) => {
          chapterDataHTML += `
					<div class="location-button">
					    <a href="#" onClick="updateSelectedChapter('${chapter.Name}','${
                chapter.ID
              }','${chapter.FbURL}','${
                chapter.Flag
              }')" class="location-link w-inline-block">
					        <div class="text-block-10">${parseEmoji(chapter.Flag)} ${
                    chapter.Name
                  }</div>
					    </a>
					</div>
      ...
      document.getElementById("chapter-regions-wrapper").innerHTML +=
        chapterDataHTML;
    })
    .catch((error) => {
      console.warn(error);
...
$(document).ready(function () {
  ...
  document.getElementById(
    "find-chapter-btn",
  ).innerHTML = `<i class="fas fa-map-marker-alt"></i>&nbsp;&nbsp;Find my chapter`;
...
function updateSelectedChapter(
  ...
  document.getElementById("selected-chapter").innerHTML =
    parseEmoji(chapterFlag) + " " + chapterName;
  ...
  document.cookie = `chapterName=${encodeURIComponent(
    chapterName,
  )}; max-age=${maxAge}; path=/`;
  document.cookie = `chapterID=${encodeURIComponent(
    chapterID,
  )}; max-age=${maxAge}; path=/`;
  document.cookie = `fbURL=${encodeURIComponent(
    facebookURL,
  )}; max-age=${maxAge}; path=/`;
  document.cookie = `chapterFlag=${encodeURIComponent(
    chapterFlag,
  )}; max-age=${maxAge}; path=/`;
  // update FB button
  ...
  document.getElementById(
    "view-fb-btn",
  ).innerText = `View ${chapterName} on Facebook`;
  ...
  fetch(
    ...
    .then((eventData) => {
      ...
      eventData.events.forEach((event) => {
        ...
        let localStartDate = new Date(event.StartTime).toLocaleDateString(
          undefined,
          options,
        );
        ...
        if (event.EventbriteURL)
          rsvpBlock += `  |  <a href="${event.EventbriteURL}" style="display: inline; font-size: 16px;" class="link" target="_blank">RSVP on Eventbrite</a>`;
        document.getElementById("event-items-wrapper").innerHTML += `
				<div class="w-dyn-item">
			        <div class="event-div">
...
function eventImageError(image) {
  console.log(
    "Event image failed to load! Someone probably deleted an event or made it private.",
  );
  //console.log(image);
...
document
  .getElementById("find-chapter-btn")
  ...
  fetch(`https://adb.dxe.io/chapters/${lat},${lng}`)
    ...
    .then((data) => {
      ...
      if (data[0].Distance > 100) {
        document
          .getElementById("event-items-wrapper")
          .insertAdjacentHTML(
            "beforeBegin",
            `<div style="padding-bottom: 30px;"><h3>The closest chapter to you is ${Math.round(
              data[0].Distance,
            )} miles away.</h3><p>Fill out <a href="https://adb.dxe.io/international" target="_blank">this form</a> to start a chapter of your own!</p></div>`,
          );
      }
...
function onGeolocateError(error) {
  ...
  if (error.code === 1) {
    // they said no
    document.getElementById("find-chapter-btn").innerHTML =
      "You denied location sharing.";
    return;
  }
  if (error.code === 2) {
...
function showChapterSelector() {
  ...
  $(".select-location-custom").fadeTo(200, 0.99);
}
...
document.querySelector("head").innerHTML +=
  "<style>img.emoji { width: 35px; }</style>";
